...
public class InitializeShipperAndroidDriver {
	...
	public static AndroidDriver<MobileElement> init(){
		...
		try {
			...
			File appShipper = new File(appDir, "Shipper_test.apk");// 司机版

			DesiredCapabilities capabilities2 = new DesiredCapabilities();
			capabilities2.setCapability(MobileCapabilityType.APPIUM_VERSION, "1.5.3");
			capabilities2.setCapability(MobileCapabilityType.PLATFORM_VERSION, "5.0.2");// 设置安卓的系统版本
			...
			driver = new AndroidDriver<MobileElement>(new URL("http://127.0.0.1:4723/wd/hub"), capabilities2);// 若存在错误则抛出异常
			// 对上面的设置是否成功进行断言
